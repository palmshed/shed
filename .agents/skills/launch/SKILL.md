...
"$LAUNCH" -- <workspace-path>                # forward extra args to shed.sh
...
- **Renderer ESM errors / `import { Menu } from 'electron'`** - `ELECTRON_RUN_AS_NODE` is set in your env. The launcher unsets it for the child, but if you spawn `shed.sh` yourself, do the same.
...
- **`launch.sh` exits non-zero with a log tail** - either pre-launch failed, `shed.sh` died before CDP came up, or CDP never opened within 90s. The tail printed to stderr is from `runDir/code.log` - read it to diagnose.
