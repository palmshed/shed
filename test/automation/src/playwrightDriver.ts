...
export class PlaywrightDriver {
	...
	private static readonly shedToPlaywrightKey: { [key: string]: string } = {
		cmd: 'Meta',
		ctrl: 'Control',
		shift: 'Shift',
		enter: 'Enter',
	...
	async sendKeybinding(keybinding: string, accept?: () => Promise<void> | void) {
		...
		for (let i = 0; i < chords.length; i++) {
			...
			for (let i = 0; i < keys.length; i++) {
				if (keys[i] in PlaywrightDriver.shedToPlaywrightKey) {
					keys[i] = PlaywrightDriver.shedToPlaywrightKey[keys[i]];
				}
