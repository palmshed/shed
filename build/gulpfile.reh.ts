...
const serverWithWebResourceIncludes = [
	...serverResourceIncludes,
	'out-build/vs/shed/browser/workbench/*.html',
...
function packageTask(type: string, platform: string, arch: string, sourceFolderName: string, destinationFolderName: string) {
	...
	return () => {
		...
		const localWorkspaceExtensions = glob.sync('extensions/*/package.json')
			...
			.filter(name => name !== 'shed-api-tests' && name !== 'shed-test-resolver'); // Do not ship the test extensions
		...
		if (type === 'reh-web') {
			...
			'resources/server/shed-192.png',
			'resources/server/shed-512.png',
		...
		if (platform === 'win32') {
			result = es.merge(result,
				gulp.src('resources/server/bin/remote-cli/shed.cmd', { base: '.' })
				...
				gulp.src('resources/server/bin/shed-server.cmd', { base: '.' })
					.pipe(rename(`bin/${product.serverApplicationName}.cmd`)),
			);
		} else if (platform === 'linux' || platform === 'alpine' || platform === 'darwin') {
			result = es.merge(result,
				gulp.src(`resources/server/bin/remote-cli/${platform === 'darwin' ? 'shed-darwin.sh' : 'shed-linux.sh'}`, { base: '.' })
					.pipe(replace('@@VERSION@@', version))
					.pipe(replace('@@COMMIT@@', commit || ''))
					.pipe(replace('@@APPNAME@@', product.applicationName))
				...
				gulp.src(`resources/server/bin/helpers/${platform === 'darwin' ? 'browser-darwin.sh' : 'browser-linux.sh'}`, { base: '.' })
					...
					.pipe(util.setExecutableBit()),
				gulp.src(`resources/server/bin/${platform === 'darwin' ? 'shed-server-darwin.sh' : 'shed-server-linux.sh'}`, { base: '.' })
