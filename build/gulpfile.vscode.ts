...
const vscodeResourceIncludes = [
	...
	// Workbench
	'out-build/vs/shed/electron-browser/workbench/workbench.html',
...
const vscodeResources = [
	...
	// Excludes
	'!out-build/vs/shed/browser/**',
	'!out-build/vs/editor/standalone/**',
	'!out-build/vs/shed/**/*-dev.html',
	...
	optimize.bundleTask(
		{
			...
			esm: {
				...
				skipTSBoilerplateRemoval: entryPoint => entryPoint === 'vs/shed/electron-browser/workbench/workbench' || entryPoint === 'vs/sessions/electron-browser/sessions'
			}
		}
	)
));
...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		const checksums = computeChecksums(out, [
			...
			'vs/shed/electron-browser/workbench/workbench.html',
			'vs/shed/electron-browser/workbench/workbench.js',
		...
		const license = gulp.src([product.licenseFileName, 'legal/ThirdPartyNotices.txt', 'licenses/**'], { base: '.', allowEmpty: true });

		// TODO the API should be copied to `out` during compile, not here
		const api = gulp.src('src/shed-dts/vscode.d.ts').pipe(rename('out/shed-dts/vscode.d.ts'));
		...
		if (platform === 'win32') {
			...
			'resources/win32/shed_70x70.png',
			'resources/win32/shed_150x150.png'
			], { base: '.' }));
		} else if (platform === 'linux') {
			const policyDest = gulp.src('.build/policies/linux/**', { base: '.build/policies/linux' })
				.pipe(rename(f => f.dirname = `policies/${f.dirname}`));
			all = es.merge(all, gulp.src('resources/linux/shed.png', { base: '.' }), policyDest);
		} else if (platform === 'darwin') {
			const shortcut = gulp.src('resources/darwin/bin/shed.sh')
				.pipe(replace('@@APPNAME@@', product.applicationName))
				.pipe(replace('@@NAME@@', product.nameShort))
				.pipe(rename('bin/shed'));
		...
		if (platform === 'linux') {
			result = es.merge(result, gulp.src('resources/completions/bash/shed', { base: '.' })
				.pipe(replace('@@APPNAME@@', product.applicationName))
				.pipe(rename(function (f) { f.basename = product.applicationName; })));

			result = es.merge(result, gulp.src('resources/completions/zsh/_shed', { base: '.' })
				.pipe(replace('@@APPNAME@@', product.applicationName))
		...
		if (platform === 'win32') {
			...
			if (versionedResourcesFolder) {
				result = es.merge(result, gulp.src('resources/win32/versioned/bin/shed.cmd', { base: 'resources/win32/versioned' })
					.pipe(replace('@@NAME@@', product.nameShort))
					.pipe(replace('@@VERSIONFOLDER@@', versionedResourcesFolder))
					.pipe(rename(function (f) { f.basename = product.applicationName; })));

				result = es.merge(result, gulp.src('resources/win32/versioned/bin/shed.sh', { base: 'resources/win32/versioned' })
					.pipe(replace('@@NAME@@', product.nameShort))
				...
				result = es.merge(result, gulp.src('resources/win32/bin/shed.cmd', { base: 'resources/win32' })
					.pipe(replace('@@NAME@@', product.nameShort))
					.pipe(rename(function (f) { f.basename = product.applicationName; })));

				result = es.merge(result, gulp.src('resources/win32/bin/shed.sh', { base: 'resources/win32' })
					.pipe(replace('@@NAME@@', product.nameShort))
					.pipe(replace('@@PRODNAME@@', product.nameLong))
					.pipe(replace('@@VERSION@@', version))
					.pipe(replace('@@COMMIT@@', String(commit)))
			...
			result = es.merge(result, gulp.src('resources/linux/bin/shed.sh', { base: '.' })
				.pipe(replace('@@PRODNAME@@', product.nameLong))
				.pipe(replace('@@APPNAME@@', product.applicationName))
	...
	return async () => {
		...
		const patchPromises = deps.map<Promise<unknown>>(async dep => {
			...
			await rcedit(fullPath, {
				...
				'version-string': {
					'CompanyName': 'Palmshed',
					'FileDescription': product.nameLong,
					'FileVersion': packageJson.version,
					'InternalName': basename,
					'LegalCopyright': 'Copyright (C) 2026 Palmshed. All rights reserved',
