...
export function createSandboxLines(sandboxingOptions: ISandboxingOnOptions): string[] {
	...
	const lines = [
		...
		'- If a command needs sandboxed write access to specific file paths outside workspace, pass requestFileValidationCheck with those paths. Shed checks sandbox access before execution and returns Access Denied without running the command when access is unavailable.',
	];
