...
suite('AgentHostSessionTaskRunner', () => {
	...
	test('local agent-host sessions apply OS-specific command overrides', async () => {
		...
		const task: ITaskEntry = {
			...
			type: 'shell',
			command: './scripts/shed.sh',
			windows: { command: '.\\scripts\\shed.bat' },
			args: ['--agents'],
		};

		(await runner.runTask(task, session))?.dispose();

		const expectedCommand = osToTaskTargetOS(OS) === 'windows' ? '.\\scripts\\shed.bat' : './scripts/shed.sh';
	...
	test('expands ${workspaceFolder} to the session working directory', async () => {
		...
		assert.deepStrictEqual(sentText, [{
			text: `./scripts/shed.sh --user-data-dir=${cwd.path}/.profile-oss`,
			shouldExecute: true,
		}]);
		assert.deepStrictEqual(resolverCalls, ['./scripts/shed.sh', '--user-data-dir=${workspaceFolder}/.profile-oss']);
	});
	...
	test('remote agent-host sessions expand ${workspaceFolder} from the POSIX host path without the renderer resolver', async () => {
		...
		assert.deepStrictEqual(sentText, [{
			text: `./scripts/shed.sh --user-data-dir=${innerCwd.path}/.profile-oss`,
			shouldExecute: true,
		}]);
