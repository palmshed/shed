...
async function startup(codeCachePath: string | undefined, nlsConfig: INLSConfiguration): Promise<void> {
	...
	// Load Main
	await import('./vs/shed/electron-main/main.js');
...
function createDefaultArgvConfigSync(argvConfigPath: string): void {
	try {
		...
		const defaultArgvConfigContent = [
			'// This configuration file allows you to pass permanent command line arguments to Shed.',
			'// Only a subset of arguments is currently supported to reduce the likelihood of breaking',
			'// the installation.',
			'//',
			'// PLEASE DO NOT CHANGE WITHOUT UNDERSTANDING THE IMPACT',
			'//',
			'// NOTE: Changing this file requires a restart of Shed.',
			'{',
			'	// Use software rendering instead of hardware accelerated rendering.',
			'	// This can help in cases where you see rendering issues in Shed.',
...
function getArgvConfigPath(): string {
	const shedPortable = process.env['VSCODE_PORTABLE'];
	if (shedPortable) {
		return path.join(shedPortable, 'argv.json');
	}
	...
	// Otherwise we configure the crash reporter from product.json
		...
		if (appCenter) {
			...
			if (crashReporterId && uuidPattern.test(crashReporterId)) {
				...
				// For Shed this is ExtensionHost process currently.
