# Shed Copilot Instructions

## Project Overview

Shed is built with a layered architecture using TypeScript, web APIs and Electron, combining web technologies with native app capabilities. The codebase is organized into key architectural layers:

### Root Folders
- `src/`: Main TypeScript source code with unit tests in `src/vs/*/test/` folders
- `build/`: Build scripts and CI/CD tools
- `extensions/`: Built-in extensions that ship with Shed
...
The `extensions/` directory contains first-party extensions that ship with Shed:
...
Each extension follows the standard Shed extension structure with `package.json`, TypeScript sources, and contribution points to extend the workbench through the Extension API.
...
- All files must include Palmshed copyright header
